...
class OtherEnvironmentDetails extends Component {
    constructor(props){
        ...
        this.state={
            ...
            SandBoxProcList: [], SandBoxProcQtyList: [], SandBoxMemList: [], SandBoxStrgList: [],
        ...
        this.LoadStorageOptions = this.LoadStorageOptions.bind(this);
        this.FilterProcessor_CombinedDB = this.FilterProcessor_CombinedDB.bind(this);
        this.GetProcList = this.GetProcList.bind(this);
        this.GetProcQtyList = this.GetProcQtyList.bind(this);
        this.PopulateStorageOptions = this.PopulateStorageOptions.bind(this);
   ...
   // Common Logic for Proc List, Proc Qty & Memory Calculations
    ...
    GetProcList(dCalcMemory){
      var lstProcs = [];
      try{
         if (ServerPlatform === "HPE ProLiant DL580 Gen9"){
             if (dCalcMemory > 4096){
                 lstProcs = ["No Processors found"];
             }else if (dCalcMemory > 1536 && dCalcMemory <= 2048){
                 lstProcs = ["E7-8890v4"];
             }else if (dCalcMemory > 3072 && dCalcMemory <= 4096){
                 lstProcs = ["E7-8890v4"];
             }else{
                 lstProcs = ["E7-8890v4", "E7-8880v4"];
             }
         }else{
             if (dCalcMemory > 6144){
                 lstProcs = ["No Processors found"];
             }else if (dCalcMemory > 3072){
                 // lstProcs = ["8176M"];
                 lstProcs = ["8180M"];
             }else if (dCalcMemory > 1536){
                 // lstProcs = ["8176", "8180", "8176M", "8180M"];
                 lstProcs = ["8176", "8180", "8180M"];
             }else{
                 lstProcs = ["8176", "8180"];
             }
        }
    ...
    FilterProcessor_CombinedDB(CalcMemory, strCombinedRole){
        var lstProcs = [];
        var strProcessor = '';
        if(strCombinedRole !== ''){
            switch (strCombinedRole){
                case "(Test/QA - Development - SandBox) combined" || "Test/QA - Development - SandBox" : {
                    lstProcs = this.GetProcList(parseInt(CalcMemory), 10);
                    if (lstProcs[0] !== "No Processors found"){
                        if (lstProcs.indexOf(cboTestQAProcessorList.SelectedValue) > 0){
                            if(strProcessor === '' ){
                               strProcessor = cboTestQAProcessorList.SelectedValue;
                            };
                        }else if (lstProcs.indexOf(cboDevDBProcessorList.SelectedValue) > 0){
                            if(strProcessor === '' ){
                               strProcessor = cboDevDBProcessorList.SelectedValue;
                            };
                        }else if (lstProcs.indexOf(cboSBDBProcessorList.SelectedValue) > 0){
                            if(strProcessor === '' ){
                               strProcessor = cboSBDBProcessorList.SelectedValue;
                            };
                        }else{
                            strProcessor = lstProcs[0];
                        }
                    }
                    break;
                }
                case "(Test/QA - SandBox) combined" || "Test/QA - SandBox" : {
                    
                    break;
                }
                case "(Test/QA - Development) combined" || "Test/QA - Development" : {
                    
                    break;
                }
                case "(Development - SandBox) combined" || "Development - SandBox" : {
    ...
    // PopulateStorageOptions(item, DBSize, Proc, ProcQty ){
    //     switch(item){
    //         case 'TestQA': {                
    //             // this.state.TestStrgList,
    //             break;
    //         }
    //         case 'Dev': {
    //             // this.state.DevStrgList
    //             break;
    //         }
    //         case 'SandBox': {
    //             // this.state.SandBoxStrgList
    //             break;
    //         }
    //     }
    // }

    // LoadStorageOptions(strCaption1, strCaption2, strCaption3){
    //     try{
    //         switch (strCaption1){                
    //             case "Dedicated Test/QA":{
    //                  // cmbTestQAStorage.IsEnabled = true;
    //                  this.state.TestStrgList = [];
    //                  this.state.DevStrgList = [];
    //                  this.state.SandBoxStrgList = [];
    //                  this.PopulateStorageOptions('TestQA', parseInt(this.state.TestObj['TestQADB'], 10), this.state.TestObj['TestProcessor'], this.state.TestObj['TestProcessorQty']);
    //                  // lblTestQACalcMem.Content = dTestQACalcMemory.ToString() + " GB";
    //                  break;
    //             }

    //             case "Dedicated Development":{
    //                 //  cmbTestQAStorage.IsEnabled = true;
    //                 //  if (cboDevDBProcessorQuantity.SelectedIndex < 0) cboDevDBProcessorQuantity.SelectedIndex = 0;

    //                 //  PopulateStorageOptions(dDevCalcMemory, cmbTestQAStorage, cboDevDBProcessorList.SelectedValue.ToString(), cboDevDBProcessorQuantity.SelectedValue.ToString());

    //                 //  // lblTestQACalcMem.Content = dDevCalcMemory.ToString() + " GB";
    //                 //  break;
    //             }

    //             case "Dedicated SandBox":{
    //                 //  cmbTestQAStorage.IsEnabled = true;
    //                 //  if (cboSBDBProcessorQuantity.SelectedIndex < 0) cboSBDBProcessorQuantity.SelectedIndex = 0;
    //                 //  PopulateStorageOptions(dSandBoxCalcMemory, cmbTestQAStorage, cboSBDBProcessorList.SelectedValue.ToString(), cboSBDBProcessorQuantity.SelectedValue.ToString());

    //                 //  // lblTestQACalcMem.Content = dSandBoxCalcMemory.ToString() + " GB";
    //                 //  break;
    //             }

    //             case "Dedicated or Combined":{
    //                 //  if (null == cmbTestQAStorage.ItemsSource)
    //                 //  { cmbTestQAStorage.Items.Clear(); }
    //                 //  else
    //                 //  { cmbTestQAStorage.ItemsSource = null; }

    //                 //  cmbTestQAStorage.IsEnabled = false;
    //                 //  // lblTestQACalcMem.Content = "";
    //                 //  break;
    //             }

    //             case "(Test/QA - Development - SandBox) combined":{
    //                  // cmbTestQAStorage.IsEnabled = true;
    //                  this.state.TestStrgList = [];
    //                  this.state.DevStrgList = [];
    //                  this.state.SandBoxStrgList = [];
    //                  var strProc = '';
    //                  var lstProcQty = [];
    //                  var TotalCalcMemory = parseInt(this.state.TestObj['TestQADB'], 10) + parseInt(this.state.DevObj['DevDB'], 10) + parseInt(this.state.SandBoxObj['SandBoxDB'], 10);
    //                  var strProc = this.FilterProcessor_CombinedDB(TotalCalcMemory.toString(), "Test/QA - Development - SandBox");
    //                  // lblTestQACalcMem.Content = dTotalCalcMemory.ToString() + " GB";
    //                  if (strProc !== ''){
    //                     lstProcQty = this.GetProcQtyList(TotalCalcMemory, strProc);
    //                     if (lstProcQty.indexOf(strProcQty) === -1){
    //                         strProcQty = lstProcQty[0];
    //                     }
    //                     this.PopulateStorageOptions(dTotalCalcMemory, cmbTestQAStorage, strProc, strProcQty);
    //                  }
    //                  break;
    //             }

    //             case "(Test/QA - SandBox) combined":{
    //                 //  cmbTestQAStorage.IsEnabled = true;
    //                 //  dTotalCalcMemory = dTestQACalcMemory + dSandBoxCalcMemory;
    //                 //  FilterProcessor_CombinedDB(ref strProc, dTotalCalcMemory.ToString(), "Test/QA - SandBox");
    //                 //  // lblTestQACalcMem.Content = dTotalCalcMemory.ToString() + " GB";

    //                 //  if (string.IsNullOrEmpty(strProc)) break;
    //                 //  lstProcQty = GetProcQtyList(dTotalCalcMemory, strProc);
    //                 //  if (!lstProcQty.Contains(strProcQty))
    //                 //      strProcQty = lstProcQty[0];

    //                 //  PopulateStorageOptions(dTotalCalcMemory, cmbTestQAStorage, strProc, strProcQty);
    //                 //  break;
    //             }

    //             case "(Test/QA - Development) combined":{
    //                 //  cmbTestQAStorage.IsEnabled = true;
    //                 //  dTotalCalcMemory = dTestQACalcMemory + dDevCalcMemory;
    //                 //  FilterProcessor_CombinedDB(ref strProc, dTotalCalcMemory.ToString(), "Test/QA - Development");
    //                 //  // lblTestQACalcMem.Content = dTotalCalcMemory.ToString() + " GB";
    ...
    //             case "(Development - SandBox) combined":{
    //                 //  cmbTestQAStorage.IsEnabled = true;
    //                 //  dTotalCalcMemory = dDevCalcMemory + dSandBoxCalcMemory;

    //                 //  FilterProcessor_CombinedDB(ref strProc, dTotalCalcMemory.ToString(), "Development - SandBox");
    //                 //  // lblTestQACalcMem.Content = dTotalCalcMemory.ToString() + " GB";
    ...
    //         switch (txtCaption2){
    //             case "(Test/QA - Development - SandBox) combined":{
    //                         // cmbDevStorage.IsEnabled = true;
    //                         // dTotalCalcMemory = dTestQACalcMemory + dDevCalcMemory + dSandBoxCalcMemory;
    //                         // FilterProcessor_CombinedDB(ref strProc, dTotalCalcMemory.ToString(), "Test/QA - Development - SandBox");
    //                         // lblDevCalcMem.Content = dTotalCalcMemory.ToString() + " GB";

    //                         // if (string.IsNullOrEmpty(strProc)) break;

    //                         // PopulateStorageOptions(dTotalCalcMemory, cmbDevStorage, strProc, strProcQty);
    //                         break;
    //             }
    //             case "(Test/QA - SandBox) combined":{
    //                         // cmbDevStorage.IsEnabled = true;
    ...
    //             case "Dedicated Test/QA":{
    //                         // cmbDevStorage.IsEnabled = true;
    //                         // if (cboTestQAProcessorQuantity.SelectedIndex < 0) cboTestQAProcessorQuantity.SelectedIndex = 0;

    //                         // PopulateStorageOptions(dTestQACalcMemory, cmbDevStorage, cboTestQAProcessorList.SelectedValue.ToString(), cboTestQAProcessorQuantity.SelectedValue.ToString());

    //                         // lblDevCalcMem.Content = dTestQACalcMemory.ToString() + " GB";
    //                         break;
    //             }
    //             case "Dedicated Development":{
    //                         // cmbDevStorage.IsEnabled = true;
    ...
    //                         // PopulateStorageOptions(dDevCalcMemory, cmbDevStorage, cboDevDBProcessorList.SelectedValue.ToString(), cboDevDBProcessorQuantity.SelectedValue.ToString());

    //                         // lblDevCalcMem.Content = dDevCalcMemory.ToString() + " GB";
    //                         break;
    //             }
    //             case "Dedicated SandBox":{
    //                         // cmbDevStorage.IsEnabled = true;
    ...
    //                         // PopulateStorageOptions(dSandBoxCalcMemory, cmbDevStorage, cboSBDBProcessorList.SelectedValue.ToString(), cboSBDBProcessorQuantity.SelectedValue.ToString());

    //                         // lblDevCalcMem.Content = dSandBoxCalcMemory.ToString() + " GB";
    //                         break;
    //             }
    //             case "Dedicated or Combined":{
    //                         // if (null == cmbDevStorage.ItemsSource)
    //                         // { cmbDevStorage.Items.Clear(); }
    //                         // else
    //                         // { cmbDevStorage.ItemsSource = null; }


    //                         // cmbDevStorage.IsEnabled = false;
    //                         // lblDevCalcMem.Content = "";
    //                         break;
    //             }
    //         }
    ...
    //         switch (txtCaption3){
    //             case "(Test/QA - Development - SandBox) combined":{
    //                         // cmbSBStorage.IsEnabled = true;
    //                         // dTotalCalcMemory = dTestQACalcMemory + dDevCalcMemory + dSandBoxCalcMemory;
    //                         // FilterProcessor_CombinedDB(ref strProc, dTotalCalcMemory.ToString(), "Test/QA - Development - SandBox");
    //                         // lblSandBoxCalcMem.Content = dTotalCalcMemory.ToString() + " GB";

    //                         // if (string.IsNullOrEmpty(strProc)) break;

    //                         // PopulateStorageOptions(dTotalCalcMemory, cmbSBStorage, strProc, strProcQty);
    //                         break;
    //             }
    //             case "(Test/QA - SandBox) combined":{
    //                         // cmbSBStorage.IsEnabled = true;
    ...
    //                         // PopulateStorageOptions(dTestQACalcMemory, cmbSBStorage, cboTestQAProcessorList.SelectedValue.ToString(), cboTestQAProcessorQuantity.SelectedValue.ToString());

    //                         // lblSandBoxCalcMem.Content = dTestQACalcMemory.ToString() + " GB";
    //                         break;
    //             }
    //             case "Dedicated Development":{
    //                         // cmbSBStorage.IsEnabled = true;
    ...
    //                         // PopulateStorageOptions(dDevCalcMemory, cmbSBStorage, cboDevDBProcessorList.SelectedValue.ToString(), cboDevDBProcessorQuantity.SelectedValue.ToString());

    //                         // lblSandBoxCalcMem.Content = dDevCalcMemory.ToString() + " GB";
    //                         break;
    //             }
    //             case "Dedicated SandBox":{
    //                         // cmbSBStorage.IsEnabled = true;
    ...
    //                         // PopulateStorageOptions(dSandBoxCalcMemory, cmbSBStorage, cboSBDBProcessorList.SelectedValue.ToString(), cboSBDBProcessorQuantity.SelectedValue.ToString());

    //                         // lblSandBoxCalcMem.Content = dSandBoxCalcMemory.ToString() + " GB";
    //                         break;
    //             }
    //             case "Dedicated or Combined":{
    //                         // if (null == cmbSBStorage.ItemsSource)
    //                         // { cmbSBStorage.Items.Clear(); }
    //                         // else
    //                         // { cmbSBStorage.ItemsSource = null; }

    //                         // cmbSBStorage.IsEnabled = false;
    //                         // lblSandBoxCalcMem.Content = "";
    //                         break;
    //             }
    //         }            
    //     }catch(e){ console.log(e); }
   ...
   // Common Logic for RadioButton Combinedsystems Selections
    ...
    PopulatingLables(){
        ...
        this.LoadStorageOptions(this.state.CombinedDataRow1, this.state.CombinedDataRow2, this.state.CombinedDataRow3);
    }
